...
> **R:"El número de líneas es mucho mayor porque en la etapa de preprocesamiento se denota el contenido de las directivas #include. El preprocesador copia el contenido completo de los archivos de cabecera (como stdio.h y matematica.h) dentro de mi archivo de mi programa, además de eliminar los comentarios."**
...
> **R:"Esto pasa ya que la frase "Archivo fuente principal" se ecuentra dentro de un comentario en (programa.c) y al pasar a la etapa de preprocesamiento cuando se genera el archivo (programa.i) se limpia el codigo borrando todo tipo de comentario o linea que no corresponda a lenguaje C."**
...
> **R:158:extern int __attribute__((__cdecl__)) __attribute__((__nothrow__)) __attribute__((__format__(__mingw_printf__,1,0))) __mingw_vprintf(const char*, __builtin_va_list);175: int __attribute__((__cdecl__)) __attribute__((__nothrow__)) __attribute__((__format__(__ms_printf__,1,0))) __msvcrt_vprintf(const char *, __builtin_va_list);
...
> **R:gcc -E programa.c | grep "Iniciando" Linea vacia;gcc -E -DDEBUG programa.c | grep "Iniciando" printf("[DEBUG] %s\n", ("Iniciando main")); **
...
> **R:"Estas líneas comunican el origen del código, indicando el número de línea y el archivo fuente original. La declaración de printf proviene del archivo de cabecera estándar stdio.h."**
> 
...
> **R:Estas instrucciones corresponden al prólogo de la función y al manejo de variables. Las primeras dos preparan el marco de pila (stack frame) para que la función tenga su propio espacio de memoria. Las siguientes mueven el valor de la variable global _llamadas a un registro del procesador (%eax) e incrementan su valor en 1, reflejando el contador que definimos en el código C.**
...
> **R:Porque en programa.o la función solo se usa, pero su código no está ahí (está Indefinida o Undefined). En cambio, en matematica.o la función está escrita y definida, por eso aparece como Texto o Código (T). **
...
> **R:Quedan símbolos U porque son funciones de la biblioteca dinámica del sistema (como printf que pertenece a la libc). El enlazador no copia el código de estas funciones dentro de mi archivo para no hacerlo gigante. En cambio, las resuelve el cargador dinámico (dynamic loader) justo en el momento cuando ejecuto el programa.**
...
> **R:Macro: Es un simple reemplazo de texto. Desaparece en el PREPROCESAMIENTO. No tiene verificación de tipos.

Función Real: Es un bloque de código con dirección de memoria. Desaparece (se traduce) recién en la COMPILACIÓN/ENSAMBLADO.**
...
> **R:Tipo T (Text): Es código ejecutable (instrucciones). Vive en la sección .text.

Tipo D (Data): Son variables globales inicializadas (como int x = 5;). Vive en la sección .data.**
...
> **R: EN WINDOWS
 ntdll.dll => /c/WINDOWS/SYSTEM32/ntdll.dll (0x7ffabd830000)
        ntdll.dll => /c/Windows/SysWOW64/ntdll.dll (0x771c0000)
        wow64.dll => /c/WINDOWS/System32/wow64.dll (0x7ffabd5d0000)
        wow64win.dll => /c/WINDOWS/System32/wow64win.dll (0x77130000)**

¿Por qué `libc` no hubo que especificarla explícitamente al enlazar con `gcc`?

> **R:Porque gcc la incluye automáticamente por defecto al enlazar. Es la biblioteca estándar y el compilador asume que casi cualquier programa en C la va a necesitar para funciones básicas como printf.**
